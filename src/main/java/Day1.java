...
public class Day1 {
  public int part1(int[] numbers) {
    ...
    for(int number : numbers) {
      if(integers.contains(2020 - number)) {
  ...
  public int part2(int[] numbers) {
    ...
    for (int i = 0; i < numbers.length-2; i++) {
      if (i == 0 || numbers[i] != numbers[i - 1]) {
        int lo = i+1, hi = numbers.length-1, sum = 2020 -numbers[i];
        while (lo < hi) {
          if (numbers[lo] + numbers[hi] == sum) {
            System.out.println(String.format("%s,%s,%s", numbers[i], numbers[lo], numbers[hi]));
